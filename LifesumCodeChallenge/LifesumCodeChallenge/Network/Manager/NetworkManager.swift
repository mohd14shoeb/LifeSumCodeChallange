...
struct NetworkManager: NetworkManagerProtocol {
    ...
    static let FoodAPIKey = ""
    let router = Router<FoodApi>()
    
    func getNewFoodDetails(foodID: Int, completion: @escaping (_ food: Food?,_ error: String?)->()){
        router.request(.newfood(foodID: foodID)) { data, response, error in
            ...
            if let response = response as? HTTPURLResponse {
                ...
                case .success:
                    ...
                    do {
                        ...
                        print(jsonData)
                        let apiResponse = try JSONDecoder().decode(FoodAPIResponse.self, from: responseData)
